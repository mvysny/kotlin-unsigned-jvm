...
Why this library is the way it is and not otherwise — FAQ-shaped: each entry is a question and its
current answer. Rewrite the answer when it changes; delete the entry when nobody asks any more. An
entry is earned by what it would cost to reverse — the platform, the scope, the accessor set — or by
research the next person would otherwise redo (cited as its `R_` in `research.md`). Not an entry: the
testing library, the CI host, a version bump, a build plugin — a comment at the site of the choice, or
nothing; nothing about `design/` itself. Cite by slug, `D_<slug>`, never by position;
`grep '^## D_' design/decisions.md` is the index. The first entry is the ruler: every later one trims
to its length. What the alternatives do is `research.md`; what must hold is the promises and
invariants in `AGENTS.md`; what one function does is its KDoc.

---

## D_library_exists — Why keep this library when `ByteBuffer` and a `.toUInt()` are free?

The case for retiring it is real and worth stating plainly: `ByteBuffer.wrap(bytes).order(…)` is in
the JDK, battle-tested and faster to reach for, and its absolute accessors are bit-for-bit identical
to these (`R_bytebuffer`). If writing the conversion at every call site and keeping `order()` state
straight does not bother you, "use `ByteBuffer`" is a defensible answer and this library buys you
nothing. What keeps it alive is that one cell of the survey is filled by nothing else
(`R_survey_matrix`): random access into a plain `ByteArray`, returning real Kotlin unsigned types,
with endianness an ordinary argument and no dependency. Concretely, on a mixed-endian frame:
...
**Rejected: every near miss in the survey.** Each fails on something structural rather than cosmetic.
`ByteBuffer`, the byte-array-view `VarHandle`s and `MemorySegment` have the right shape but are
signed, and the `.toUInt()` you forget is a silent bug rather than a compile error — `0xFFFFFFFF`
becomes `-1` (`R_bytebuffer`, `R_varhandle_byte_array_view`, `R_memorysegment`). kotlinx-io has the
right *types* and the wrong *shape*: a consuming cursor, pre-1.0 (`R_kotlinx_io`). Okio has no
unsigned accessors at all (`R_okio`). Netty, POI, Bouncy Castle and korlibs each bury a right-ish API
in a dependency nobody would take on for byte shuffling alone (`R_netty_bytebuf`, `R_jvm_utility_libs`,
`R_korlibs_memory`). The Kotlin stdlib's `getIntAt` is Native-only, experimental, and its byte order
is undocumented (`R_kotlin_native_byte_accessors`). Declarative parsers such as Kaitai Struct are a
different category, not a competitor (`R_format_parsers`).

**The cost of keeping it is close to zero**, which is what makes the answer easy: three files, no
dependencies, no runtime behaviour to rot, nothing to track but the JDK baseline and the Kotlin
version. The honest counterweight is a bus factor of one — a single-maintainer micro-library is a
real, if small, supply-chain consideration next to `ByteBuffer` (free, forever) or Okio.

**What it deliberately does not do:** no 24-bit accessors (still an open idea), no
half-precision (`D_no_float16`), no unit scaling (`D_no_unit_scaling`), JVM only (`D_jvm_only`), and
no slicing, bulk copy, varints, strings or off-heap. It is one thing, which is a virtue right up until
you need the second thing and reach for `ByteBuffer` anyway.

## D_jvm_only — Why stay JVM-only rather than go multiplatform?
...
stdlib is experimental with undocumented endianness — the *multiplatform* column of `R_survey_matrix`).
It is also *empty*, and this library is the obvious thing to fill it with. Declined anyway, as of
2026-09-12: the JVM is the only platform this is shipped on, so the argument is positioning, not need —
and the costs are recurring while the benefit is speculative. Revisit when a user asks for Native or JS.
...
Nothing here is a one-way door, so deferring is cheap. The one cost that does grow with time is the
coordinates — each release adds users pinned to the `-jvm` name — but the `artifactId` override above is a
real escape hatch, and Maven Central relocation POMs exist. Not enough to justify renaming today.

## D_floats_in_scope — Why does an "unsigned" library have `Float` and `Double` accessors?

`Float` has no signed/unsigned dimension at all, so "why does an *unsigned* library have floats?" is
an objection someone re-proposes every couple of years. The scope was never "unsigned only", and
floats are not what broke it: `getShort`, `getInt` and `getLong` are signed and have been from the
start. `unsigned` in the name describes **what was
missing from the alternatives** — `DataInputStream` and `ByteBuffer` widen instead of returning
unsigned types (the *Kotlin unsigned* column of `R_survey_matrix`) — not the boundary of what this
library does. What the library actually is, and what `README.md` has claimed since its first line, is
*Dart's `ByteData` for Kotlin*: random access into a plain `ByteArray` at a byte offset, with
endianness an ordinary argument. Floats are a `ByteData` accessor and fit that sentence exactly.
...
`EndianTest`'s existing `getInt`/`getLong` coverage carries the correctness weight. Delegating to
`getInt`/`getLong` is not merely convenient but exactly right: endianness applies to the *container*,
not to the number, so whole-word reversal is the entirety of it (`README.md` states it too).
...
buys eight delegating functions. And the gap was one the library's own docs kept having to concede,
so keeping it was a recurring tax paid to avoid a one-off cost.
...
scope under the *No wrapper, no dependencies* promise. `README.md` can therefore claim `ByteData`
scalar parity as fact.

## D_no_float16 — Why no half-precision accessors, neither binary16 nor bfloat16?
...
* **No JDK help at this baseline** — the one argument that expires on its own. JDK 20 added
  `Float.float16ToFloat(short)` / `floatToFloat16(float)`; this library targets 17 (and CI runs
  17/21/24), so it would be ~20 hand-rolled lines whose subnormal and overflow edges would be the only
  real correctness risk in the whole float story — everything else here is delegation to code the JDK
  already tests.
...
## D_no_unit_scaling — Why does the API stop at the bit pattern rather than offer scale, offset and units?
...
## D_varhandle — Why byte-array-view `VarHandle`s rather than the hand-rolled shift-or arithmetic?
...
single unaligned, byte-swapping load or store (`R_varhandle_byte_array_view`).
...
**The handles must be top-level `private val`s**, for the reason set out in the comment above them in
`Endian.kt`. The check that catches a regression: `javap -c` must show
`invokevirtual VarHandle.get:([BI)I`, never `:([BI)Ljava/lang/Object;`.

**Not a behaviour change.** Out-of-range offsets still throw; only the exception's *message* differs,
now counting in units of the value's width (`R_varhandle_byte_array_view`; the `Endian` KDoc documents
what a caller sees).
...
## D_float_raw_bits — Why `toRawBits()` rather than `toBits()` in the float setters?
...
by the absence of `toBits` from the source, which `design/verify_project_tripwires.sh` checks, not by a
...
## D_patch_module — Why `--patch-module` the Kotlin output rather than fake the package with an empty `Dummy.java`?
...
The two produce an identical jar, entry for entry: `jar --describe-module` reports
`com.github.mvysny.unsigned@<version>` exporting the package and requiring `java.base` +
`kotlin.stdlib` either way.
...
## D_kdoc_voice — Why write KDoc for this API rather than keep Dart's `ByteData` prose?
...
stated once on the `Endian` class doc, and per-member blocks carrying only what is specific to them.
...
**The bounds contract goes in `@throws` tags, not in prose.** `ByteArrays.kt` documents it with
`@throws` — a documented KDoc block tag, listed in
[Kotlin's KDoc reference](https://kotlinlang.org/docs/kotlin-doc.html), which Dokka parses correctly.
It was contested only because the renderer then filling the published jar dropped it silently;
`D_dokka_html` has the measurement, and swapped the renderer.
...
banning a standard language tag permanently — with a mechanical check, no less — to route around one
broken renderer is the wrong component to change, and the **sources jar** (`withSourcesJar()`), which
is the primary documentation channel for a Kotlin consumer's IDE, rendered the tag correctly all along.

**Consequences.** `Endian`'s members are deliberately no longer standalone — they lean on the class
doc for the shared contract. The asymmetry between the two files is intentional: `Endian` has a
...
## D_dokka_html — Why fill the `-javadoc.jar` from Dokka's HTML renderer rather than its javadoc one?
...
shipped first and used until 2026-09-12. Both formats satisfy Central, which only checks that a signed
`-javadoc.jar` exists and never looks inside; the jar keeps its name and classifier either way. The
javadoc format was picked for the layout other tooling expects — `element-list`,
...
**Consequences.** Anyone wanting javadoc `-link` against this library is worse off, per the table;
accepted. The `@throws` tags now reach the published jar unaided, which is what lets `D_kdoc_voice`
keep the bounds contract in a tag rather than in prose.
